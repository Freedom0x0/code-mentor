...
| 「本次会话结束」「收工」 | 🔴 沉淀本次进度快照（见下方 Session Resume）|
| 「继续工作」「接着上次」 | 🔴 读最新快照报 1 句话摘要（见下方 Session Resume）|
...
| 「会话结束前自动写快照吧」 | 错。Session Resume **用户口令触发**，Claude 不主动提示打快照，避免打断心流 |

## v2.1 新增能力

### 🔄 Session Resume（工作续接）

会话边界能力 —— 你收工时沉淀进度快照，下次开新 session 续接，**工作不中断**。

| 时机 | 你说 | Claude 做 |
|---|---|---|
| 收工时 | 「本次会话结束」「今天工作结束」「收工」「先到这」 | 从对话提炼 4 字段草稿 → 展示给你 → **你确认** → 落盘 |
| 新 session | 「继续工作」「接着上次」「接上次」 | 读最新快照 → **报 1 句话进度** → 等你决定下一步 |
...
**关键行为**:
- 🔴 **不主动**：Claude 不主动提示打快照（避免打断心流），不主动在「继续工作」后跑 4 步
- **改文件前要确认**：写侧「确认保存吗」复用既有红线
- **历史无限保留**：`session-snapshots/` 目录只增不删，你自己定期清理
- **不强存**：草稿被拒 3 轮不强存，对话太短 / 没改东西不强写空快照

**文件位置**（不进项目目录，跟现有运行时档案一致）：
```
~/.claude/code-mentor/<目录名-哈希6位>/
  session-snapshot.md           # 永远最新
  session-snapshots/<ISO>.md    # 历史快照，无限保留
```
...
| 收尾沉淀触发 | `knowledge` | 业务/技术组合/项目骨架入库到 Obsidian vault |
