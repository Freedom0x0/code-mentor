...
4. 收尾 — 改完必走，不可省略
        └─ 🔁 验证闭环（用户跑过 + 贴输出 才算完成）
        └─ 📝 收尾沉淀（JOURNAL / bug 库 / DECISIONS，均先问用户）
```
...
经 darwin-skill 独立子 agent 盲测（2026-07-27），v1.0/v1.1/v1.2 共 10 个核心场景全部 PASS，dim8 实测得分 8/10。v2.0 14 个新场景待独立盲测补齐。
...
```
code-mentor/
  SKILL.md          # skill 主文件（唯一真相来源，371 行）
  README.md         # 本文件
  evals/
    evals.json      # 回归测试集
  tests/
